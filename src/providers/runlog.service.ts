...
export class RunLogService {
     ...
     * @param reportDate
     */
    public prevLogDetail(stationId: string, endDate: string): Observable<PrepLogDetailResp> {
        let url = this.prevLogDetailUrl + '?stationId=' + stationId + '&endDate=' + endDate;
        return this.http.get(url);
    }
    /**
     * 新增日志
    ...
    public getRecord(recordId, stationId) {
        let url =
            this.getRecordUrl + '/' + recordId + (recordId === 0 ? '?stationId=' + stationId : '');
        return this.http.get(url);
    }

    public saveRecord(params) {
        let url = this.saveRecordUrl;
        return this.http.post(url, params, { responseType: ResponseContentType.Text });
    }

    public updateRecord(params) {
        let url = this.saveRecordUrl;
        return this.http.put(url, params, { responseType: ResponseContentType.Text });
    }
